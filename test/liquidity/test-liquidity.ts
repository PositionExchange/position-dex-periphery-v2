...
export class TestLiquidity {
    ...
    async addLiquidity(amountVirtual: StringOrNumber, indexPip: StringOrNumber, asset: string, idSender: number, opts: CallOptions = {}): Promise<number> {
        ...
        await this.dexNFT.connect(this.users[idSender]).addLiquidity({
            pool: this.mockMatching.address,
            amountVirtual: toWei(amountVirtual),
            indexedPipRange: indexPip,
            isBase: asset.toLowerCase() == "base"
        });
        return 0;
    }
